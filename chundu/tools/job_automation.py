        ...
        try:
            if self.provider == "openai":
                response = self.client.chat.completions.create(
                    model="gpt-4o", messages=[{"role": "user", "content": prompt}], max_tokens=1000, temperature=0.7
                )
...
{job.company}. With over 18 years of experience in cloud infrastructure, AI/ML \
platforms, and DevOps leadership, I am confident in my ability to make significant \
contributions to your team.

In my current role as Cloud AI Architect, I have led enterprise-scale cloud \
migrations, implemented Kubernetes platforms serving hundreds of microservices, \
and built GPU infrastructure for AI/ML workloads. My expertise in \
{", ".join(self.profile.skills[:4])} aligns well with your requirements.
...
I am excited about the opportunity to bring my technical expertise and leadership \
experience to {job.company}. I would welcome the chance to discuss how my \
background can contribute to your team's success.
...
"""
        ...
        try:
            if self.provider == "openai":
                response = self.client.chat.completions.create(
                    model="gpt-4o", messages=[{"role": "user", "content": prompt}], max_tokens=800, temperature=0.7
                )
                return response.choices[0].message.content
            elif self.provider == "anthropic":
                response = self.client.messages.create(
...
def main():
    ...
    parser.add_argument(
        "--keywords", "-k", default="Cloud Architect DevOps Platform Engineering", help="Search keywords"
    )
    parser.add_argument("--days", "-d", type=int, default=7, help="Number of days to look back")
    parser.add_argument(
        "--generate", "-g", action="store_true", help="Generate cover letters and tailored resumes for top matches"
    )
    parser.add_argument("--top", "-t", type=int, default=5, help="Number of top jobs to generate documents for")
    parser.add_argument("--output", "-o", default="job_results", help="Output directory")
